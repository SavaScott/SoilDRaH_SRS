...
figure3.df <- read_csv(file = figure3.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  mutate(across(c(`x value`,`DOC (mg g^-1)`),
                as.numeric)) #make things numbers
         
ggplot(data = figure3.df |>
         filter(`graph label` == "a"), 
       mapping = aes(x = `x value`, y = `DOC (mg g^-1)`, group = `litter type`, color = `litter type`)) +
  geom_line() +
  geom_point() +
    labs(x = "Time (days)",
      caption = read_csv(file = figure3.file, #read in that file again but...
                           n_max = 1, #only read the first line
                           col_names = 'caption', #call it the caption
                           #read everything as a character
                           col_types = cols(.default = col_character()),
