...
figure2.file <- 'Wang2017_Figure2_al.csv'
figure3.file <- 'Wang2017_Figure3_al.csv'
...
- [Wang2017_Figure2_al.csv](Wang2017_Figure2_al.csv)
  + Figure 2 from primary article
- [Wang2017_Figure3_al.csv](Wang2017_Figure3_al.csv)
...
+ Lead
  ...
  - [ ] Create a fresh fork of the primary repository https://github.com/ktoddbrown/SoilDRaH/tree/main, it might be useful to rename it 'SoilDRaH_Wang2017'
  - [ ] Create a copy of this template folder and rename it to Wang2017
...
+ Do
  ...
  - [ ] Extract points from Figure3 and export it to csv
...
plot.df <- read_csv(file = figure2.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  mutate(across(c(`Depth [cm]`, `Soil organic carbon density [g cm$-1$]`),
                as.numeric)) #make things numbers
         
ggplot(data = plot.df, 
       mapping = aes(x = `Depth [cm]`, y = `Soil organic carbon density [g cm$-1$]`, color=Site)) +
  geom_line() +
  geom_point() +
  #depth below surface is often plotted flipped and downward positive
  scale_x_reverse() +
  coord_flip() +
  labs(caption = read_csv(file = figure2.file, #read in that file again but...
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Wang2017_Figure2_JK.csv Wang2017_Figure2_al.csv
```
...
plot.df <- read_csv(file = figure3.file, #read in the specific file
         skip = 1, #skip the first line with the caption
         #read everything as a character, don't worry about formatting yet
         col_types = cols(.default = col_character())) |>
  mutate(across(c(`Depth [cm]`, `Soil organic carbon density [g cm$-1$]`),
...
ggplot(data = plot.df, 
  ...
  scale_x_reverse() +
  coord_flip() +
  labs(caption = read_csv(file = figure3.file, #read in that file again but...
                           n_max = 1, #only read the first line
                           col_names = 'caption', #call it the caption
                           #read everything as a character
                           col_types = cols(.default = col_character()),
...
diff --strip-trailing-cr --suppress-common-lines -y temp/Wang2017_Figure3_JK.csv Wang2017_Figure3_al.csv
```
...
# You will want to repeat the line below to cross check each csv and md file in the
#... data rescue. Having no lines flagged by this command will ensure that each character matches exactly.
diff --strip-trailing-cr --suppress-common-lines -y temp/Wang2017_Methods_ABC.md Wang2017_Methods.md
```
