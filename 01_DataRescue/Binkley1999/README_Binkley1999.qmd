...
mutate(
  value = ifelse(
    column_name == "Age (mo)",
    ifelse(
      str_detect(with_entry, "(?i)change"), "Change",
      with_entry),
NA_character_
     ),
  lowerbound = as.character(
    ifelse(
    column_name == "Age (mo)" & str_detect(with_entry, "to"),
   (str_extract(with_entry, "^(\\d+)\\s*to")),
    NA_real_
)),
  upperbound = as.character(
    ifelse(
    column_name == "Age (mo)" & str_detect(with_entry, "to"), 
     (str_extract(with_entry, "(?<=to\\s*)(\\d+)")),
      NA_real_
    )),
  mean = as.character(
...
#Still working on table 1, got all age_id's to show but missing lower- and upperbound
